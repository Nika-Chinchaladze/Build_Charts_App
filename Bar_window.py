...
class IronMan(QMainWindow):
    def __init__(self):
        ...
        self.w_label = self.findChild(QLabel, "w_label")
        ...
        self.w_box = self.findChild(QComboBox, "w_box")
        ...
        self.back_button.clicked.connect(self.Return_Backward)
        self.display_button.clicked.connect(self.Draw_Bar)
    ...
    def Draw_Bar(self):
        ...
        try:
            ...
            if len(number_array) == len(label_array):
                plt.bar(label_array, number_array, color = f"{color_type}", width = bar_width)
